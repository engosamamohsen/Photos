...
android {
  ...
  buildTypes {
    getByName("debug") {
      ...
      buildConfigField("String", "APIKEY", Config.Environments.API_KEY)
    }

    signingConfigs {
      create("releaseConfig") {
        storeFile = file(rootProject.file("/PAYMOB"))
        storePassword = "paymob2022"
        keyAlias = "paymob"
        keyPassword = "paymob2022"
      }
    }
...
//      resValue("string", "google_api_key", gradleLocalProperties(rootDir).getProperty("GOOGLE_API_KEY"))
      ...
      buildConfigField("String", "ROOM_DB", Config.Environments.roomDb)
      buildConfigField("String", "API_KEY", Config.Environments.API_KEY)
    }
  }
